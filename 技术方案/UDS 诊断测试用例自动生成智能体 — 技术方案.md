...
|                 | 第 1 次调用（参数提取）           | 第 2 次调用（用例生成）               |
| --------------- | ----------------------- | --------------------------- |
| **输入**          | Excel 原始文本（openpyxl 读取） | 3 层提示词（Base + Service + 参数） |
| **任务**          | 结构化数据抽取 → JSON          | 按规则生成测试用例 → 文本              |
| **temperature** | 0.05（极低，保证确定性）          | 0.05（极低，保证确定性）              |
| **max_tokens**  | ~8,000                  | ~16,000 ~ 32,000            |
| **耗时预估**        | 2-5 秒                   | 10-60 秒                     |
...
| **LLM 调用**   | httpx + OpenAI 兼容协议     | 多提供商自动故障转移（OpenRouter → 智谱 → DeepSeek → Qwen），无需额外框架                 |
...
**LLM 瓶颈**：真正的瓶颈不在 Python 侧，而在 LLM API 的并发限额。当前使用云端 API（OpenRouter/智谱），单 key 并发约 5~10 请求。多 Worker 场景下需确认 API 套餐的 RPM 限制，必要时升级套餐或部署本地 vLLM。
...
class FullExtractionResult(BaseModel):
    ...
    dtc_list: list[DTCEntry]        # DTC列表（0x14/0x19用）
...
| 特性       | 说明                                       |
| -------- | ---------------------------------------- |
| **多提供商** | 支持 OpenRouter、智谱 GLM、DeepSeek、通义千问、本地 vLLM      |
...
[pipeline] LLM 第1次调用 — 参数提取
        ...
        │  包含: basic_info + App/Boot双域矩阵 + 安全映射 + ...
        │  耗时: 2-5秒
        ▼
    Pydantic 校验
...
[test_parser] 输出解析
        │  解析分类、用例、步骤
        │  填充 author/design_method/precondition 等配置字段
        ▼
SSE 推送结果 → 前端展示
        │
        ▼
...
| 文件 | 内容 | 估算 Token |
| --- | --- | --- |
| `prompts/service_0x10_prompt.md` | 0x10/0x11 规则 | ~5,200 |
| `prompts/service_0x11_prompt.md` | 0x11 规则 | ~2,300 |
| `prompts/service_0x14_prompt.md` | 0x14 规则（4分类） | ~1,600 |
| `prompts/service_0x19_prompt.md` | 0x19 规则（5子功能） | ~1,500 |
| `prompts/service_0x22_prompt.md` | 0x22 规则（DID遍历） | ~1,300 |
| `prompts/service_0x27_prompt.md` | 0x27 规则（8分类） | ~2,400 |
| `prompts/service_0x28_prompt.md` | 0x28 规则（双遍历） | ~1,900 |
| `prompts/service_0x2E_prompt.md` | 0x2E 规则（写DID） | ~1,400 |
| `prompts/service_0x31_prompt.md` | 0x31 规则（RID条件） | ~1,600 |
| `prompts/service_0x3E_prompt.md` | 0x3E 规则（S3定时器） | ~1,800 |
| `prompts/service_0x85_prompt.md` | 0x85 规则（DTC开关） | ~2,100 |
| `prompts/extract_prompt.md` | LLM 参数提取系统提示词 | ~800 |
| **提示词资产总计** | | **~27,900** |

### 10.2 3层提示词组装结构

```
Layer 1: SERVICE PROMPT (每个服务独立，~1,300 ~ 5,200 tokens)
  ├─ 服务概述（SID/正负响应格式/子功能/典型NRC）
  ├─ 分类1: Session Layer Test
  ├─ 分类2: SPRMIB Test（如适用）
  ├─ 分类3: Secure Access Test（如适用）
  ├─ 分类4~N: 服务特有分类
  └─ 分类N+1: Incorrect Command Test
...
| `src/uds_agent/generate_pipeline.py` | 生成管道编排 | 153 行 |
| `src/uds_agent/pipeline.py` | 提取管道编排 | 141 行 |
| `src/uds_agent/llm_client.py` | 多提供商 LLM 网关 | 275 行 |
| `src/uds_agent/prompt_loader.py` | 提示词组装 + 参数格式化 | 252 行 |
...
| `src/uds_agent/schemas.py` | 提取结果 Pydantic 模型 | 95 行 |
...
| 所有 LLM 提供商不可用         | 系统无法工作        | 多提供商故障转移（OpenRouter → 智谱 → DeepSeek → Qwen） |
